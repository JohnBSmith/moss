...
fn RJ(mut x: f64, mut y: f64, mut z: f64, mut p: f64) -> f64 {
    ...
    x.powf(-3.0/2.0)*(4f64).powi(-n) + 6.0*s
}
...
fn eiE(phi: f64, m: f64) -> f64 {
    ...
    let c = (phi).cos();
    let mss = m*s*s;
    s*RF(c*c, 1.0-mss, 1.0) - 1.0/3.0*mss*s*RJ(c*c, 1.0-mss, 1.0, 1.0)
}
...
fn eiPi(phi: f64, n: f64, m: f64) -> f64 {
    ...
    let mss = m*s*s;
    let nss = n*s*s;
    s*RF(c*c, 1.0-mss, 1.0) + 1.0/3.0*nss*s*RJ(c*c, 1.0-mss, 1.0, 1.0-nss)
}

fn legendre_rec(n: i32, m: i32, x: f64) -> f64{
    if n == m {
        SQRT_PI/gamma(0.5 - float(n))*(2.0*(1.0 - x*x).sqrt()).powi(n)
    } else if n-1 == m {
        x*float(2*n-1)*legendre_rec(m,m,x)
    } else {
...
fn hermite(n: i32, x: f64) -> f64 {
    if n<2 {
        if n == 1 {2.0*x} else if n == 0 {1.0} else {NAN}
    } else {
        let mut a = 1.0;
        let mut b = 2.0*x;
        for k in 1..n {
...
fn chebyshev1(n: i32, x: f64) -> f64 {
    if n<2 {
        if n == 0 {1.0} else if n == 1 {x} else {NAN}
    } else {
        let mut a = 1.0;
        let mut b = x;
        for _ in 1..n {
...
fn chebyshev2(n: i32, x: f64) -> f64 {
    if n<2 {
        if n == 0 {1.0} else if n == 1 {2.0*x} else {NAN}
    } else {
        let mut a = 1.0;
        let mut b = 2.0*x;
        for _ in 1..n {
...
fn upper_gamma(s: f64, z: f64) -> f64{
    if s + 4.0 < z {
        z.powf(s)*upper_gamma_cf(s,z,40)
    } else {
        gamma(s) - z.powf(s)*lower_gamma_series(s,z,60)
    }
}

fn lower_gamma(s: f64, z: f64) -> f64 {
    if s + 4.0 < z {
        gamma(s) - z.powf(s)*upper_gamma_cf(s,z,40)
    } else {
        z.powf(s)*lower_gamma_series(s,z,60)
    }
}
...
fn hurwitz_zeta(s: f64, a: f64) -> f64 {
    ...
    y + Npa.powf(1.0-s)/(s-1.0) + 0.5*Npa.powf(-s)
...
fn zeta(s: f64) -> f64 {
    if s > 60.0 {
        ...
        a*gamma(1.0 - s)*hurwitz_zeta(1.0 - s, 1.0)
    }
}
...
fn czeta_em(s: C64) -> C64 {
    ...
    y + (1.0-s).expf(Nf)/(s-1.0) + 0.5*(-s).expf(Nf)
...
fn czeta(s: C64) -> C64 {
    if s.re > -1.0 {
        ...
        a*cgamma(1.0-s)*czeta_em(1.0-s)
    }
}

fn bernoulli_number(n: f64) -> f64 {
    if n == 0.0 {1.0} else {-n*zeta(1.0-n)}
}

fn beta(x: f64, y: f64) -> f64 {
    let s = sgngamma(x)*sgngamma(y)*sgngamma(x + y);
    s*f64::exp(lgamma(x) + lgamma(y) - lgamma(x + y))
}
