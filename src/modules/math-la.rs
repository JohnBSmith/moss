...
impl Interface for Array {
            ...
            match indices[0] {
                Object::Int(i) => {
                    ...
                    if i >= 0 && (i as usize) < self.s[0].shape {
                        Ok(a[(self.base as isize + self.s[0].stride*i) as usize].clone())
                    } else {
                        env.index_error("Index error in a[i]: out of bounds.")
                    }
                },
                ref i => {
                    env.type_error1("Type error in a[i]: i is not an integer.", "i", i)
                }
            }
        } else if self.n == 2 {
            if indices.len() == 1 {
                ...
                if i >= 0 && (i as usize) < self.s[0].shape {
                    ...
                    Ok(Object::Interface(a))
                } else {
                    env.index_error("Index error in a[i]: i is out of bounds.")
                }
                   ...
                   j>=0 && (j as usize)<self.s[1].shape
                {
                    ...
                    env.index_error("Index error in a[i,j]: out of bounds.")
                }
            } else {
                env.index_error("Index error in a[...]: shape does not fit.")
            }
    ...
    fn set_index(&self, indices: &[Object], value: &Object, env: &mut Env) -> FnResult {
        if self.n == 1 && indices.len() == 1 {
            ...
            if indices.len() == 1 {
                ...
                let v = if let Some(x) = downcast::<Array>(value) {x} else {
    ...
    fn mul(self: Rc<Self>, b: &Object, env: &mut Env) -> FnResult {
        if self.n == 1 {
            ...
            scalar_multiplication(env, b, &self)
        } else if self.n == 2 {
            if let Some(b) = downcast::<Array>(b) {
                if b.n == 1 {
...
fn scalar_matrix(n: usize, x: &Object, zero: &Object) -> Rc<Array> {
    let mut acc: Vec<Object> = Vec::with_capacity(n*n);
    for i in 0..n {
        for j in 0..n {
            acc.push(if i == j {x.clone()} else {zero.clone()});
        }
    }
    Array::matrix(n,n,acc)
}
...
) -> Result<(),Box<Exception>>
{
    ...
    if depth + 1 == a.n {
        ...
        for i in 0..shape {
            let index = base+(i as isize)*stride;
            map_unary_operator_rec(env, buffer, a, index,
                data, depth + 1, operator)?;
        }
    }
    Ok(())
}
...
{
    if a.n == 1 {
        let mut acc: Vec<Object> = Vec::with_capacity(a.s[0].shape);
        let stride = a.s[0].stride;
        let base = a.base as isize;
        let data = a.data.borrow();
        for i in 0..a.s[0].shape {
            let x = &data[(base + i as isize*stride) as usize];
            acc.push(operator(env,x)?);
        }
        Ok(Object::Interface(Array::vector(acc)))
    } else if a.n == 2 {
        let m = a.s[0].shape;
        let n = a.s[1].shape;
        ...
        let mut acc: Vec<Object> = Vec::with_capacity(m*n);
        let base = a.base as isize;
        let data = a.data.borrow();
        for i in 0..m {
        ...
        Ok(Object::Interface(Array::matrix(m,n,acc)))
    } else {
        ...
        let mut s = a.s.clone();
        setup_strides(&mut s);
        Ok(Object::Interface(Array::array(s,buffer)))
    }
}
...
) -> Result<(),Box<Exception>>
{
    ...
    if depth + 1 == a.n {
        ...
        for i in 0..ashape {
            ...
            map_binary_operator_rec(env, buffer,
                a, aindex, adata, b, bindex, bdata,
                depth + 1, operator)?;
        }
    }
    Ok(())
...
fn shape_compare(a: &Array, b: &Array) -> bool {
    ...
    a.s.iter().zip(b.s.iter()).all(|t| t.0.shape == t.1.shape)
}
...
) -> FnResult
{
    if a.n == 1 {
        ...
        if let Some(b) = downcast::<Array>(b) {
            ...
            map_binary_operator_rec(env, &mut buffer,
                a, abase, adata, b, bbase, bdata, 0, operator)?;
            let mut s = a.s.clone();
            setup_strides(&mut s);
            Ok(Object::Interface(Array::array(s, buffer)))
        } else {
...
fn list(a: &Array) -> Vec<Object> {
    if a.n == 1 {
        ...
        for i in 0..m {
            let jbase = base + i as isize*istride;
            let mut acc: Vec<Object> = Vec::with_capacity(n);
            for j in 0..n {
                let x = &data[(jbase + j as isize*jstride) as usize];
                acc.push(x.clone());
            }
            buf.push(Object::from(acc));
        }
        buf
    } else {
        panic!();
...
) -> FnResult
{
    ...
    let mut acc: Vec<Object> = Vec::with_capacity(m);
    for i in 0..m {
...
fn shape(a: &Array) -> FnResult {
    ...
    let mut acc: Vec<Object> = Vec::with_capacity(n);
    for i in 0..n {
        acc.push(Object::Int(a.s[i].shape as i32));
    }
...
fn unit_vector(n: usize, k: usize) -> Rc<Array> {
    let mut acc: Vec<Object> = Vec::with_capacity(n);
    for i in 0..n {
        acc.push(Object::Int(if i == k {1} else {0}));
    }
    Array::vector(acc)
}

fn vector(_env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    Ok(Object::Interface(Array::vector(Vec::from(argv))))
}
...
fn matrix(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    ...
    env.type_error("Type error in matrix(*args): expected args of type List.")
}
...
fn array_from(dim: usize, a: &Object) -> FnResult {
    ...
    Ok(Object::Interface(Rc::new(Array {
...
fn array(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    ...
    if n == 1 {
        if let Object::List(a) = list_obj {
            Ok(Object::Interface(Array::vector(a.borrow().v.clone())))
        } else {
            env.type_error("Type error in array(n,a): a is not a list")
        }
    } else {
        array_from(n, &list_obj)
    }
}
...
fn scalar(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    ...
    let e = &argv[1];
    let z = &argv[2];
    Ok(Object::Interface(scalar_matrix(n,e,z)))
}
...
fn diag(_env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    ...
    for i in 0..n {
        for j in 0..n {
            if i == j {
                acc.push(argv[k].clone());
                k += 1;
            } else {
                acc.push(Object::Int(0));
            }
        }
    }
    Ok(Object::Interface(Array::matrix(n,n,acc)))
}
...
fn diag_slice(env: &mut Env, a: &Array) -> FnResult {
    ...
    if a.n == 2 && n == a.s[1].shape {
        ...
        for i in 0..n {
            ...
            acc.push(data[index].clone());
        }
        Ok(List::new_object(acc))
    } else {
        env.type_error("Type error in A.diag: A is not a square matrix.")
    }
}
...
fn la_copy(env: &mut Env, _pself: &Object, argv: &[Object]) -> FnResult {
    ...
    Ok(if let Some(a) = downcast::<Array>(&argv[0]) {
        Object::Interface(copy(a))
    } else {
        argv[0].clone()
    })
}
...
pub fn load_math_la(env: &mut Env) -> Object
{
    ...
    interface_types_set(env.rte(), interface_index::POLY_ARRAY,
        type_array.clone());
