    ...
    match *x {
        ...
        Object::String(ref s) => {
            ...
            let f = Box::new(move |_env: &mut Env, _pself: &Object, _argv: &[Object]| -> FnResult{
                Ok(if index == s.data.len() {
                    Object::empty()
                } else {
                    index += 1;
                    Object::String(Rc::new(CharString {
    ...
    match argv.len() {
        ...
        1 => {
            match argv[0] {
                ...
                _ => env.type_error("Type error in i.list(n): n is not an integer.")
            }
        },
        n => env.argc_error(n,1,1,"list")
    }
}
...
fn sort_by(env: &mut Env, a: &mut [Object], fcmp: &Object) -> Option<FnResult> {
    ...
    let cmp = |x: &Object, y: &Object| -> Ordering {
        ...
        if value {Ordering::Less} else {Ordering::Greater}
    };
    a.sort_by(cmp);
    }
    if let Some(e) = err {Some(e)} else {None}
}
...
fn until(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    ...
    let g = Box::new(move |env: &mut Env, _pself: &Object, argv: &[Object]| -> FnResult {
        ...
        let x = env.call(&i, &Object::Null, &[])?;
        if x.is_empty() {Ok(x)} else {
            let y = env.call(&f, &Object::Null, &[x.clone()])?;
            match y {
                Object::Bool(y) => {
                    Ok(if y {Object::empty()} else {x})
                },
                ref y => env.type_error1(
...
fn enumerate(env: &mut Env, pself: &Object, argv: &[Object]) -> FnResult {
    ...
    let g = Box::new(move |env: &mut Env, _pself: &Object, argv: &[Object]| -> FnResult {
        ...
        let x = env.call(&i, &Object::Null, &[])?;
        Ok(if x.is_empty() {x} else {
            k += 1;
            List::new_object(vec![Object::Int(k - 1), x])
        })
    });
    Ok(new_iterator(g))
}
